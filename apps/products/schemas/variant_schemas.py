...
def serialize_variant(
    ...
    data = {
        ...
        # "stock": variant.stock,  # REMOVED - moved to admin block
    ...
    if is_admin:
        data.update({
            ...
            "stock": variant.stock,  # MOVED HERE - admin only
            "weight": float(variant.weight) if variant.weight is not None else None,
            "height": float(variant.height) if variant.height is not None else None,
            "width": float(variant.width) if variant.width is not None else None,
            "depth": float(variant.depth) if variant.depth is not None else None,
            "dimensions": {
                "weight": float(variant.weight) if variant.weight is not None else None,
                "height": float(variant.height) if variant.height is not None else None,
                "width": float(variant.width) if variant.width is not None else None,
            ...
            "gross_profit": float(variant.gross_profit),
            "margin_percentage": round(variant.margin_percentage, 2),
            "markup_percentage": round(variant.markup_percentage, 2),
            "inventory_cost_value": float(variant.inventory_cost_value),
            "potential_revenue": float(variant.potential_revenue),
            "potential_profit": float(variant.potential_profit),
        })
    else:
        # Public-facing stock info (no raw numbers)
