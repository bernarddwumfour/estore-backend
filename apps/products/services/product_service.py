...
class ProductService:
    ...
    def get_product_detail(slug: str, is_admin: bool = False) -> Optional[Dict]:
        ...
        product_data["related_products"] = serialize_product_list(
            related, is_admin=is_admin
        )
    ...
    def bulk_action_products(
        ...
        if action in ["publish", "draft", "archive"]:
            ...
            for product_id in product_ids:
                try:
                    ...
                    if (
                        new_status == Product.STATUS_PUBLISHED
                        and not product.published_at
                    ):
                        from django.utils import timezone
            ...
            for product_id in product_ids:
                try:
                    ...
                    product.save()
                    results["success"].append(
                        {"id": product_id, "name": product.title, "is_new": is_new}
                    )
                except Product.DoesNotExist:
                    results["failed"].append(
                        {
        ...
        logger.info(
            f"Bulk {action} action performed by admin {user.email}: {len(results['success'])} succeeded, {len(results['failed'])} failed"
        )
    ...
    def create_product(
        ...
        try:
            ...
            product = Product.objects.create(
                ...
                published_at=(
                    timezone.now()
                    if data.get("status") == Product.STATUS_PUBLISHED
                    else None
                ),
            )
